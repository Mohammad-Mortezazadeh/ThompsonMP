...
!!
      subroutine mp_thompson_init(ncol, nlev, con_g, con_rd, con_eps,      &
         ...
         real(4),           intent(in   ) :: con_g, con_rd, con_eps
         ...
         real(4),           intent(inout) :: spechum(:,:)
         real(4),           intent(inout) :: qc(:,:)
         real(4),           intent(inout) :: qr(:,:)
         real(4),           intent(inout) :: qi(:,:)
         real(4),           intent(inout) :: qs(:,:)
         real(4),           intent(inout) :: qg(:,:)
         real(4),           intent(inout) :: ni(:,:)
         real(4),           intent(inout) :: nr(:,:)
         ! Aerosols
         logical,                   intent(in   ) :: is_aerosol_aware
         logical,                   intent(in   ) :: merra2_aerosol_aware
         real(4),           intent(inout), optional :: nc(:,:)
         real(4),           intent(inout), optional :: nwfa(:,:)
         real(4),           intent(inout), optional :: nifa(:,:)
         real(4),           intent(inout), optional :: nwfa2d(:)
         real(4),           intent(inout), optional :: nifa2d(:)
         real(4),           intent(in)    :: aerfld(:,:,:)
         ! State variables
         real(4),           intent(in   ) :: tgrs(:,:)
         real(4),           intent(in   ) :: prsl(:,:)
         real(4),           intent(in   ) :: phil(:,:)
         ...
         real(4),           intent(in   ), optional :: diag3d(:,:,:)
         ! CCPP error handling
         character(len=*),          intent(  out) :: errmsg
         integer,                   intent(  out) :: errflg

         !
         real(4) :: qv(1:ncol,1:nlev)       ! kg kg-1 (water vapor mixing ratio)
         real(4) :: hgt(1:ncol,1:nlev)      ! m
         real(4) :: rho(1:ncol,1:nlev)      ! kg m-3
         real(4) :: orho(1:ncol,1:nlev)     ! m3 kg-1
         real(4) :: nc_local(1:ncol,1:nlev) ! needed because nc is only allocated if is_aerosol_aware is true
         !
         real (kind=4) :: h_01, z1, niIN3, niCCN3
         integer :: i, k
         ...
         if (is_aerosol_aware .and. merra2_aerosol_aware) then
            write(errmsg,'(*(a))') "Logic error: Only one Thompson aerosol option can be true, either is_aerosol_aware " // &
         ...
         if (convert_dry_rho) then
           ...
           if (is_aerosol_aware .or. merra2_aerosol_aware) then
              nc = nc/(1.0_4-spechum)
              nwfa = nwfa/(1.0_4-spechum)
              nifa = nifa/(1.0_4-spechum)
           end if
         end if
         ...
         where(qi .GT. 0 .and. ni .LE. 0.0) ni = make_IceNumber(real(qi*rho, kind=kind_phys), real(tgrs, kind=kind_phys)) * orho
         ...
         where(qr .GT. 0 .and. nr .LE. 0.0) nr = make_RainNumber(real(qr*rho, kind=kind_phys), real(tgrs, kind=kind_phys)) * orho
         ...
         if (is_aerosol_aware) then
           ...
           where(qc .LE. 0.0) nc=0.0
            where(qc .GT. 0 .and. nc .LE. 0.0) nc = make_DropletNumber( &
               real(qc*rho, kind=kind_phys), &
               real(nwfa*rho, kind=kind_phys)) * orho
...
!>@{
      subroutine mp_thompson_run(ncol, nlev, con_g, con_rd,        &
         ...
         integer,                   intent(in   ) :: ncol
         integer,                   intent(in   ) :: nlev
         real(4),           intent(in   ) :: con_g
         real(4),           intent(in   ) :: con_rd
         real(4),           intent(in   ) :: con_eps
         ! Hydrometeors
         logical,                   intent(in   ) :: convert_dry_rho
         real(4),           intent(inout) :: spechum(:,:)
         real(4),           intent(inout) :: qc(:,:)
         ...
         logical,                   intent(in)    :: merra2_aerosol_aware
         real(4), optional, intent(inout) :: nc(:,:)
         real(4), optional, intent(inout) :: nwfa(:,:)
         real(4), optional, intent(inout) :: nifa(:,:)
         real(4), optional, intent(in   ) :: nwfa2d(:)
         real(4), optional, intent(in   ) :: nifa2d(:)
         real(4),           intent(in)    :: aerfld(:,:,:)
         logical,         optional, intent(in   ) :: aero_ind_fdb
         ! State variables and timestep information
         real(4),           intent(inout) :: tgrs(:,:)
         real(4),           intent(in   ) :: prsl(:,:)
         real(4),           intent(in   ) :: phii(:,:)
         real(4),           intent(in   ) :: omega(:,:)
         integer,                   intent(in   ) :: islmsk(:)
         real(4),           intent(in   ) :: dtp
         ...
         real(4),           intent(inout) :: prcp(:)
         real(4),           intent(inout), optional :: rain(:)
         real(4),           intent(inout), optional :: graupel(:)
         real(4),           intent(inout), optional :: ice(:)
         real(4),           intent(inout), optional :: snow(:)
         real(4),           intent(  out) :: sr(:)
         ! Radar reflectivity
         real(4),           intent(inout) :: refl_10cm(:,:)
         real(4),           intent(inout) :: max_hail_diam_sfc(:)
         ...
         logical,                   intent(in)    :: ext_diag
         real(4), target,   intent(inout), optional :: diag3d(:,:,:)
         ...
         real(4),           intent(in), optional :: spp_wts_mp(:,:)
         real(4),           intent(in), optional :: spp_prt_list(:)
         character(len=10),         intent(in), optional :: spp_var_list(:)
         real(4),           intent(in), optional :: spp_stddev_cutoff(:)

         logical, intent (in) :: cplchm
         ! ice and liquid water 3d precipitation fluxes - only allocated if cplchm is .true.
         real(kind=4), intent(inout), dimension(:,:), optional :: pfi_lsan
         real(kind=4), intent(inout), dimension(:,:), optional :: pfl_lsan
         ...
         real(4) :: qv(1:ncol,1:nlev)               !< kg kg-1
         ! Vertical velocity and level width
         real(4) :: w(1:ncol,1:nlev)                !< m s-1
         real(4) :: dz(1:ncol,1:nlev)               !< m
         ! Rain/snow/graupel fall amounts
         real(4) :: rain_mp(1:ncol)                 ! mm, dummy, not used
         real(4) :: graupel_mp(1:ncol)              ! mm, dummy, not used
         real(4) :: ice_mp(1:ncol)                  ! mm, dummy, not used
         real(4) :: snow_mp(1:ncol)                 ! mm, dummy, not used
         real(4) :: delta_rain_mp(1:ncol)           ! mm
         real(4) :: delta_graupel_mp(1:ncol)        ! mm
         real(4) :: delta_ice_mp(1:ncol)            ! mm
         real(4) :: delta_snow_mp(1:ncol)           ! mm

         real(4) :: pfils(1:ncol,1:nlev,1)
         real(4) :: pflls(1:ncol,1:nlev,1)
         ! Radar reflectivity
         ...
         !real(4), dimension(:,:,:), pointer :: vts1       => null()
         !real(4), dimension(:,:,:), pointer :: txri       => null()
         !real(4), dimension(:,:,:), pointer :: txrc       => null()
         real(4), dimension(:,:,:), pointer :: prw_vcdc   => null()
         real(4), dimension(:,:,:), pointer :: prw_vcde   => null()
         real(4), dimension(:,:,:), pointer :: tpri_inu   => null()
         real(4), dimension(:,:,:), pointer :: tpri_ide_d => null()
         real(4), dimension(:,:,:), pointer :: tpri_ide_s => null()
         real(4), dimension(:,:,:), pointer :: tprs_ide   => null()
         real(4), dimension(:,:,:), pointer :: tprs_sde_d => null()
         real(4), dimension(:,:,:), pointer :: tprs_sde_s => null()
         real(4), dimension(:,:,:), pointer :: tprg_gde_d => null()
         real(4), dimension(:,:,:), pointer :: tprg_gde_s => null()
         real(4), dimension(:,:,:), pointer :: tpri_iha   => null()
         real(4), dimension(:,:,:), pointer :: tpri_wfz   => null()
         real(4), dimension(:,:,:), pointer :: tpri_rfz   => null()
         real(4), dimension(:,:,:), pointer :: tprg_rfz   => null()
         real(4), dimension(:,:,:), pointer :: tprs_scw   => null()
         real(4), dimension(:,:,:), pointer :: tprg_scw   => null()
         real(4), dimension(:,:,:), pointer :: tprg_rcs   => null()
         real(4), dimension(:,:,:), pointer :: tprs_rcs   => null()
         real(4), dimension(:,:,:), pointer :: tprr_rci   => null()
         real(4), dimension(:,:,:), pointer :: tprg_rcg   => null()
         real(4), dimension(:,:,:), pointer :: tprw_vcd_c => null()
         real(4), dimension(:,:,:), pointer :: tprw_vcd_e => null()
         real(4), dimension(:,:,:), pointer :: tprr_sml   => null()
         real(4), dimension(:,:,:), pointer :: tprr_gml   => null()
         real(4), dimension(:,:,:), pointer :: tprr_rcg   => null()
         real(4), dimension(:,:,:), pointer :: tprr_rcs   => null()
         real(4), dimension(:,:,:), pointer :: tprv_rev   => null()
         real(4), dimension(:,:,:), pointer :: tten3      => null()
         real(4), dimension(:,:,:), pointer :: qvten3     => null()
         real(4), dimension(:,:,:), pointer :: qrten3     => null()
         real(4), dimension(:,:,:), pointer :: qsten3     => null()
         real(4), dimension(:,:,:), pointer :: qgten3     => null()
         real(4), dimension(:,:,:), pointer :: qiten3     => null()
         real(4), dimension(:,:,:), pointer :: niten3     => null()
         real(4), dimension(:,:,:), pointer :: nrten3     => null()
         real(4), dimension(:,:,:), pointer :: ncten3     => null()
         real(4), dimension(:,:,:), pointer :: qcten3     => null()
         ...
         if (first_time_step .and. istep==1 .and. blkno==1) then
            ...
            if (nsteps>1 .and. dt_inner < dtp) then
               ...
               write(*,'(a,i0,a,a,f6.2,a)') 'Thompson MP is using ', nsteps, ' substep(s) per time step with an ', &
                                            'effective time step of ', dtp/real(nsteps, kind=4), ' seconds'
               ...
               write(*,'(a,i0,a,a,f6.2,a)') 'Thompson MP is using ', ndt, ' inner loops per time step with an ', &
                                            'effective time step of ', dtp/real(ndt, kind=4), ' seconds'
            end if
         end if
         ...
         prcp    = prcp    + max(0.0, delta_rain_mp/1000.0_4)
         graupel = graupel + max(0.0, delta_graupel_mp/1000.0_4)
         ice     = ice     + max(0.0, delta_ice_mp/1000.0_4)
         snow    = snow    + max(0.0, delta_snow_mp/1000.0_4)
         rain    = rain    + max(0.0, (delta_rain_mp - (delta_graupel_mp + delta_ice_mp + delta_snow_mp))/1000.0_4)
...
!!
      ...
      subroutine get_niwfa(aerfld, nifa, nwfa, ncol, nlev)
         ...
         real (kind=4), dimension(:,:,:), intent(in)  :: aerfld
         real (kind=4), dimension(:,:),   intent(out ):: nifa, nwfa
